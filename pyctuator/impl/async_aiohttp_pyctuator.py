...
class AsyncAioHttpPyctuator(PyctuatorRouter):
    def __init__(self, app: web.Application, pyctuator_impl: AsyncPyctuatorImpl, disabled_endpoints: Endpoints) -> None:
        ...
        async def get_environment(request: web.Request) -> web.Response:
            env_data = await pyctuator_impl.get_environment()
            return web.json_response(env_data, dumps=custom_dumps)

        async def get_info(request: web.Request) -> web.Response:
            return web.json_response(pyctuator_impl.get_app_info(), dumps=custom_dumps)

        async def get_health(request: web.Request) -> web.Response:
            health = await pyctuator_impl.get_health()
        ...
        async def get_metric_measurement(request: web.Request) -> web.Response:
            metric = await pyctuator_impl.get_metric_measurement(request.match_info["metric_name"])
            return web.json_response(metric, dumps=custom_dumps)
