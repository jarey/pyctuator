...
class AsyncPyctuatorImpl:
    ...
    def __init__(
        ...
        self.metrics_providers: List[AsyncMetricsProvider] = []
        self.health_providers: List[AsyncHealthProvider] = []
        self.environment_providers: List[AsyncEnvironmentProvider] = []
    ...
    def register_health_providers(self, provider: AsyncHealthProvider) -> None:
        self.health_providers.append(provider)

    def register_environment_provider(self, provider: AsyncEnvironmentProvider) -> None:
    ...
    async def get_environment(self) -> EnvironmentData:
        active_profiles: List[str] = []
        property_sources = []
        for source in self.environment_providers:
            property_sources.append(await source.get_properties_source(self.secret_scrubber))
        
        env_data = EnvironmentData(active_profiles, property_sources)
        return env_data
    ...
    async def get_health(self) -> HealthSummary:
        health_statuses: Mapping[str, HealthStatus] = {}
        
        for provider in self.health_providers:
            if provider.is_supported():
                health_status = await provider.get_health()
                health_statuses[provider.get_name()] = health_status
    ...
    async def get_metric_measurement(self, metric_name: str) -> Metric:
        for provider in self.metrics_providers:
            if metric_name.startswith(provider.get_prefix()):
                return await provider.get_metric(metric_name)
